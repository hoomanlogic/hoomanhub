...
class Action(TimeStampedModel, UniquelyNamedModel, ArchiveableModel, DocumentableModel, TaggableModel):
    STATUS = ((0, 'Outstanding'), (1, 'Future'), (2, 'Done'), (3, 'Abandoned'))
...
class Target(TimeStampedModel, ArchiveableModel):
    ...
    MEASURE_TYPE = ((0, 'Executions'), (1, 'Progress'), (2, 'Time'))
    measure = models.PositiveSmallIntegerField(choices=MEASURE_TYPE, default=0)
    met_after = models.IntegerField(default=1)
    ...
    def todate(dateobj):
        ...
        elif isinstance(dateobj, tuple):
            from time import mktime
            return datetime.fromtimestamp(mktime(dateobj[0])).date()
        else:
            raise Exception("Unrecognized type.")

    def text_to_data(self):
        periodfrom = datetime.today().date()
        periodto = None

        modifier = 0
        if 'next' in self.text.split(' '):
            modifier = 1
        elif ['last', 'previous'] in self.text.split(' '):
            modifier = -1

        if 'week' in self.text.split(' '):
            periodfrom += timedelta(days=-periodfrom.weekday(), weeks=modifier)
            periodto = periodfrom + timedelta(weeks=1, days=-1)

        elif 'month' in self.text.split(' '):
            # modify referenced month and set to first day of the month
            if 0 < (periodfrom.month + modifier) < 13:
                periodfrom = Target.todate("{}-{:0>2}-{:0>2}".format(periodfrom.year, periodfrom.month + modifier, 1))
            elif (periodfrom.month + modifier) < 1:
                periodfrom = Target.todate("{}-{:0>2}-{:0>2}".format(periodfrom.year - 1, 12, 1))
            elif (periodfrom.month + modifier) > 12:
                periodfrom = Target.todate("{}-{:0>2}-{:0>2}".format(periodfrom.year + 1, 1, 1))

            # get end of month from first of the month
            if (periodfrom.month + 1) < 13:
                periodto = Target.todate("{}-{:0>2}-{:0>2}".format(periodfrom.year, periodfrom.month + 1, periodfrom.day))
            else:
                periodto = Target.todate("{}-{:0>2}-{:0>2}".format(periodfrom.year + 1, 1, periodfrom.day))
            periodto += timedelta(days=-1)

        elif 'year' in self.text.split(' '):
            periodfrom = Target.todate("{}-{:0>2}-{:0>2}".format(periodfrom.year + modifier, 1, 1))
            periodto = Target.todate("{}-{:0>2}-{:0>2}".format(periodfrom.year, 12, 31))

        elif ['between', 'and'] in self.text.split(' '):
            rangetext = self.text.replace('between', '')
            import parsedatetime as pdt
            pdc = pdt.Calendar()

            periodfrom = Target.todate(pdc.parse(rangetext.split('and')[0]))
            periodto = Target.todate(pdc.parse(rangetext.split('and')[1]))

        else:
            import parsedatetime as pdt
            pdc = pdt.Calendar()
            periodto = Target.todate(pdc.parse(self.text))

        self.periodfrom = periodfrom
        self.periodto = periodto

    #MEASURE_TYPE = ((0, 'executions'), (1, 'percentage'), (2, 'minutes'))
    #FREQUENCY = ((0, 'yearly'), (1, 'monthly'), (2, 'weekly'), (3, 'daily'), (4, 'dates'))
    #dates = models.TextField(blank=True, null=True)
    #freq = models.PositiveSmallIntegerField(choices=FREQUENCY)
    #measure = models.PositiveSmallIntegerField(choices=MEASURE_TYPE, default=0)
    #interval = models.PositiveSmallIntegerField(default=1)
    #starts = models.DateField()
    #off_interval = models.PositiveSmallIntegerField(null=True, blank=True)
    #period_target = models.IntegerField(null=True, blank=True)
    #met_after = models.IntegerField(null=True, blank=True)
    #break_after = models.PositiveSmallIntegerField(null=True, blank=True)
    #break_freq = models.PositiveSmallIntegerField(choices=FREQUENCY, null=True, blank=True)
    #break_interval = models.PositiveSmallIntegerField(null=True, blank=True)
    #
    ...
    #    for dt in self.dates.split():
    #        if Calendar.parse(dt) >= relativedate:
    #            return dt
    #    return None
    #
